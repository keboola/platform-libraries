...
- **GitHub releases** — `.github/actions/create-release` creates the release in the standalone repo, with notes written from the pull request descriptions behind that version by the Claude Code CLI (prompt in `prompt.md`, context gathered by `bin/release-context.sh`). `release.yml` calls it after the split; `create-release.yml` (`workflow_dispatch`, `dry-run` defaulting to **true**) is for retries and for backfilling versions tagged before this existed. Both skip a release that already exists, so both are safe to re-run. The mechanics are commented in the action.
...
- **`create-release` writes notes from pull request bodies, which are attacker-influenced input.** Treat the `--allowed-tools` list as a security boundary, not tidiness: it deliberately excludes `gh api`, which takes arbitrary URLs and would give injected text a way out with the Anthropic key and workflow token in scope. Everything allowed is either local (`Read`/`Grep`/`Glob`) or a read-only `gh pr` subcommand; the notes are the process stdout, so the model needs no write access at all. The residual risk is bad *note text*, which a human sees in the release.
