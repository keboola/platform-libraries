...
class SyncActionsApiClientTest extends TestCase
{
    ...
    public function testCallActionPreservesObjectFidelity(): void
    {
        ...
        $body = '{"filled": {"x": "y"}, "empty": {}, "nested": {"inner": {}}, "indexed": {"0": "a"}}';
        $requestHandler = self::createRequestHandler($requestsHistory, [new Response(200, [], $body)]);
        ...
        // an integer-keyed object would be renumbered to a JSON array under an associative decode
        self::assertInstanceOf(stdClass::class, $response->data->indexed);
        self::assertSame('a', $response->data->indexed->{'0'});
    }
    ...
    public function testInvalidJsonResponseIsWrappedAsClientException(): void
    {
        ...
        try {
            ...
            self::assertStringContainsString('Response is not valid JSON: Syntax error', $e->getMessage());
            self::assertSame(200, $e->getStatusCode());
            self::assertSame('{not a valid json]', $e->getResponseBody());
        }
    }
