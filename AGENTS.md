...
- **GitHub releases** — `.github/actions/create-release`, called from `release.yml` after the split and from `create-release.yml` (`workflow_dispatch`: `library`, `version`, `dry-run` defaulting to **true**) for retries and for backfilling versions tagged before this existed. It skips if the release already exists, so both callers are safe to re-run. `bin/release-context.sh` gathers the deterministic part — previous released version, the commits in that range touching `libs/<lib>`, and the pull requests those commits came from — then `anthropics/claude-code-action@v1` turns the **pull request descriptions** into the notes and the action appends the `**Full Changelog**` compare link in shell. **Nothing degrades quietly:** empty notes fail the job, and so do an empty commit list or a failed pull request lookup in `bin/release-context.sh` — a context file with nothing in it would still yield *some* text, which would pass the emptiness check and ship. A wrong release note is worse than a missing one; recovery is a `create-release.yml` run. Only strict `<lib>/X.Y.Z` tags get a release — `<lib>/build-*` and `<lib>/dev-*` tags are still published, just without one.
...
- **The previous release tag cannot be found with `git tag --sort=-v:refname`.** `<lib>/*` also matches throwaway build/dev tags (`output-mapping/build-pat-664.1`), which that sort puts on top. `bin/release-context.sh` filters to strict `^<lib>/[0-9]+\.[0-9]+\.[0-9]+$` and takes the entry directly below the released one under `sort -V`.
