...
both to a Manage token, so `scopes`/`isSuperAdmin` checks are identical. The `ManageApiTokenAuth`
attribute is a deprecated 1:1 alias kept for backwards compatibility — use
`KubernetesServiceAccountAuth` instead.
...
* to use `KubernetesServiceAccountAuth` (or the deprecated `ManageApiTokenAuth`), install `keboola/kbc-manage-api-php-client`
