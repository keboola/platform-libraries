...
- **GitHub releases** — `.github/actions/create-release` creates the release in the library's standalone repo, with notes generated from the pull requests behind that version. `release.yml` calls it on a version tag; `create-release.yml` is the manual path, for retries and for backfilling a version tagged before this existed.
...
- **Credentials are action inputs, never job-level `env`.** A composite action substitutes inputs into the steps that name them instead of exporting them, so each credential reaches only the step that uses it; job-level `env` would put every one of them in every step's environment.
- **Keep release-notes generation toolless.** Its input is contributor-written pull request bodies, and one completion with no tools and no network gives injected text nothing to reach.
...
- Release notes: `RELEASE_GENERATOR_GEMINI_API_KEY` (secret) plus the `RELEASE_GENERATOR_GEMINI_MODEL` repository **variable**, which holds the model id — there is no default in code, so an unset variable fails the release-notes step.
