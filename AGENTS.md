...
- **GitHub releases** — `.github/actions/create-release`, called from `release.yml` after the split and from `create-release.yml` (`workflow_dispatch`: `library`, `version`, `dry-run` defaulting to **true**) for retries and for backfilling versions tagged before this existed. It skips if the release already exists, so both callers are safe to re-run. `bin/release-context.sh` gathers the deterministic part — previous released version, the commits in that range touching `libs/<lib>`, and the pull requests those commits came from — then `anthropics/claude-code-action@v1` turns the **pull request descriptions** into the notes and the action appends the `**Full Changelog**` compare link in shell. **No fallback body:** empty notes fail the job, because a wrong release note is worse than a missing one; recovery is a `create-release.yml` run. Only strict `<lib>/X.Y.Z` tags get a release — `<lib>/build-*` and `<lib>/dev-*` tags are still published, just without one.
...
- **The publish credentials sit in job-level `env`, which the Claude Code subprocess inherits.** That is why `create-release` grants a **narrow `--allowedTools`** (specific `gh`/`git` subcommands plus `Read`/`Grep`/`Glob`/`Write`) — nothing that can dump the environment. Widen it only with that in mind.
