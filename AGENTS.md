...
- **Adding a new library** requires: a `lib-<lib>.yml`, a test job + a `tests-result` `needs:` entry in `ci.yml`, a `libs/<lib>/AGENTS.md` (+ the `CLAUDE.md` pointer), and — only if its standalone repo name differs from the dir — an entry in the `split-library` action's `case`.
