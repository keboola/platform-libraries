...
Guidance for AI coding agents working in this repository. Tool-specific entry points (`CLAUDE.md`) point here.

Per-library guidance is in `libs/<library>/AGENTS.md`.
...
Each library has a `libs/<lib>/AGENTS.md` covering its architecture, commands and gotchas, plus a `libs/<lib>/CLAUDE.md` that only points at it. The tool-neutral `AGENTS.md` name is what Codex, Cursor, Gemini CLI and others look for; the `CLAUDE.md` files exist so Claude Code finds them too. This file is the root equivalent, and the root `CLAUDE.md` is its pointer.

`libs/<lib>/AGENTS.md` is the **only** guidance file that reaches a library's standalone repo — the split takes `libs/<lib>/` and nothing above it. It therefore opens with a self-contained section telling an agent working in that read-only mirror that pull requests belong here, in `keboola/platform-libraries`. Keep that section intact when editing.
...
4. **`tests-result`** — barrier job (`needs:` `detect-changes` + all 22, `if: always()`) that fails if any of those results is not `success`/`skipped`. `detect-changes` is included so a failed gate (which would *skip* every test job) can't slip through as `success` and leave the PR mergeable with nothing tested. **This is the single required status check** for branch protection.
5. **`publish`** — **one matrix job** (`matrix.lib: ${{ fromJSON(needs.detect-changes.outputs.libs) }}`, `fail-fast: false`) that runs on every green build (`needs.tests-result.result == 'success'`, guarded by `libs != '[]'`) and splits/publishes each affected library to its standalone repo. Runs on any branch, so pushing a dev branch makes `composer require keboola/<lib>:dev-<branch>` work in dependents. Only the branch being built is pushed (a release build pushes only its single new tag) — never all branches or all tags (see gotchas).
...
- **Tag releases** — `.github/workflows/release.yml` (`on: push: tags: ['*/*']`). **One job** that derives the library from the tag (`<lib>/<version>` → `<lib>`), skips cleanly if it is not a library dir, publishes via the same composite action, and then creates the GitHub release. **No tests run on a tag** (the code was already tested on the branch); tag → publish only. The `<lib>/` prefix is stripped, so `output-mapping/1.2.3` becomes `1.2.3` in the standalone repo.
- **`split-library` composite action** — `.github/actions/split-library`. Takes a single `library` input, resolves the target repo name via `bin/target-repo.sh`, mints a short-lived GitHub App installation token scoped to that one repo via `actions/create-github-app-token`, and calls `bin/split-repo.sh`.
- **`bin/target-repo.sh`** — maps a library dir to its standalone repo name. The **5 dir≠repo exceptions** live here and nowhere else: `git-service-api-client→git-service-php-api-client`, `key-generator→php-key-generator`, `query-service-api-client→query-service-api-php-client`, `sandboxes-service-api-client→sandboxes-service-api-php-client`, `vault-api-client→vault-api-php-client`; all others repo == dir. Both publishing actions call it.
- **GitHub releases** — `.github/actions/create-release`, called from `release.yml` after the split and from `create-release.yml` (`workflow_dispatch`: `library`, `version`, `dry-run` defaulting to **true**) for retries and for backfilling versions tagged before this existed. It skips if the release already exists, so both callers are safe to re-run. `bin/release-context.sh` gathers the deterministic part — previous released version, the commits in that range touching `libs/<lib>`, and the pull requests those commits came from — then `anthropics/claude-code-action@v1` turns the **pull request descriptions** into the notes and the action appends the `**Full Changelog**` compare link in shell. **The model is never asked to write from nothing:** a release whose range changed nothing under `libs/<lib>` is reported as `has-changes=false` and gets a fixed one-line note instead ("Released to pick up changes in the libraries it depends on") — that is a green job, not a bug, and it fires on the ~14-in-518 dependency-only re-releases this monorepo produces. Everything else is fatal: empty notes from the model, and a failed pull request lookup in `bin/release-context.sh`. A wrong release note is worse than a missing one; recovery is a `create-release.yml` run. Only strict `<lib>/X.Y.Z` tags get a release — `<lib>/build-*` and `<lib>/dev-*` tags are still published, just without one.
- **`bin/split-repo.sh`** — `git clone --mirror` of the checkout → `git filter-repo --subdirectory-filter libs/<lib>` (rewrites/strips tag prefix) → pushes **only the build branch** (5th arg, branch builds) **or the single new tag** (6th arg, release builds). Never `--mirror`, all branches, or all tags. Publish/release jobs check out with `fetch-depth: 0` + `fetch-tags: true`.
...
- **Publishing pushes only what the build produced — never `--mirror`/all refs.** Two reasons a full mirror fails: (1) the standalone repos have branch protection the publish App cannot bypass, so pushing every monorepo branch is rejected with `GH006: Protected branch update failed`; (2) bulk-pushing all tags re-pushes immutable release tags that already exist with different SHAs, rejected as "tag already exists". So `split-repo.sh` pushes only the build branch (`GITHUB_REF_NAME` on branch builds, force) or the single new tag (stripped `GITHUB_REF_NAME` on tag builds, no force); the `split-library` action computes which from `GITHUB_REF_TYPE`. **Side effect:** branches deleted in the monorepo are **not** pruned from the standalone repos — `cleanup-branch.yml` (`on: delete`) handles that. (Note: `git filter-repo` itself removes the `origin` remote and promotes remote-tracking refs — do not hand-manipulate refs before it runs, or `git remote add origin` later fails with "remote origin already exists".)
...
- **Credentials are action inputs, never job-level `env`.** A composite action substitutes inputs into the steps that name them instead of exporting them, so the App private key reaches only `create-github-app-token`; job-level `env` would put it in every step's environment, including Claude Code's. The Anthropic key is the one credential the model's process legitimately holds.
- **`create-release` writes notes from pull request bodies, which are attacker-influenced input.** Treat the `--allowedTools` list as a security boundary, not tidiness: it deliberately excludes `gh api`, which takes arbitrary URLs and would give injected text a way out with the Anthropic key and workflow token in scope. Everything allowed is either local (`git log`/`show`/`diff`, `Read`/`Grep`/`Glob`/`Write`) or a read-only `gh pr` subcommand. The residual risk is bad *note text*, which a human sees in the release.
- **`claude-code-action` needs `github_token` passed explicitly, and it must not be replaced with `id-token: write`.** The action resolves a token from that input or from OIDC, and from nothing else — omit it and the step dies in its Prepare phase, before the model runs. The OIDC branch exchanges for a Claude GitHub App token minted with `contents`/`pull-requests`/`issues` **write**, which would hand a write-capable token to a session reading attacker-influenced pull request bodies. Passing `${{ github.token }}` keeps it at the caller's `contents: read` + `pull-requests: read`. Related: `show_full_output` is deliberately not set — this repo is public, so that input would dump every tool result, pull request body and file read into a world-readable log; the notes are surfaced through `$GITHUB_STEP_SUMMARY` instead.
- **The previous release tag cannot be found with `git tag --sort=-v:refname`.** `<lib>/*` also matches throwaway build/dev tags (`output-mapping/build-pat-664.1`), which that sort puts on top. `bin/release-context.sh` filters to strict `^<lib>/[0-9]+\.[0-9]+\.[0-9]+$`, takes the entry directly below the released one under `sort -V`, and reports whether the released tag is the list's last entry as `is-newest`.
- **`gh release create` is always given an explicit `--latest`.** `gh` sends `make_latest` only when the flag is present, and the API defaults it to true — so backfilling an old version through `create-release.yml`, the whole point of that workflow, would move the Latest badge onto it. The value comes from `release-context.sh`'s `is-newest`.
...
- **Adding a new library** requires: a `lib-<lib>.yml`, a test job + a `tests-result` `needs:` entry in `ci.yml`, a `libs/<lib>/AGENTS.md` (+ the `CLAUDE.md` pointer), and — only if its standalone repo name differs from the dir — an entry in `bin/target-repo.sh`.
...
- Publishing: `SPLIT_APP_PRIVATE_KEY` (GitHub App private key; the App must be installed with push rights to all 22 target repos — the same `contents: write` also covers creating releases, so releases need no separate App).
- Release notes: `AI_CR_ANTHROPIC_API_KEY` — an **organization** secret shared with the other keboola repos that run Claude Code, not provisioned per repo.
